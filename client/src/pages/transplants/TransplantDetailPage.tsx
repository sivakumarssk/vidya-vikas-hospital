...
import { getTransplantBySlug } from '../../data/transplants'

export function TransplantDetailPage() {
  const { slug } = useParams()
  const programme = getTransplantBySlug(slug)

  if (!programme) return <Navigate to="/transplants" replace />

  return (
    <PageScaffold>
      <section className="bg-brand-surface py-14 sm:py-16 lg:py-20">
        <Container>
          <Link to="/transplants" className="text-sm font-semibold text-brand-green hover:underline">
            ← Back to transplant programmes
          </Link>
          ...
          <div className="mt-5 rounded-3xl bg-white p-8 shadow-lg sm:p-10">
            ...
            <h1 className="mt-4 font-heading text-4xl font-extrabold text-brand-navy sm:text-5xl">{programme.name}</h1>
            <p className="mt-5 max-w-4xl text-lg leading-relaxed text-brand-muted">{programme.shortDescription}</p>

            <div className="mt-8 grid gap-4 sm:grid-cols-3">
              <div className="rounded-2xl bg-brand-surface p-5">
                <p className="text-xs uppercase tracking-[0.08em] text-brand-muted">Hub</p>
                <p className="mt-1 font-heading text-xl font-bold text-brand-navy">Sambalpur</p>
              </div>
              <div className="rounded-2xl bg-brand-surface p-5">
                <p className="text-xs uppercase tracking-[0.08em] text-brand-muted">Network</p>
                <p className="mt-1 font-heading text-xl font-bold text-brand-navy">Kakinada · Berhampur · Jharsuguda</p>
              </div>
              <div className="rounded-2xl bg-brand-surface p-5">
      ...
      <section className="py-14 sm:py-16">
        <Container>
          ...
          <div className="mt-6 max-w-5xl space-y-4 text-base leading-8 text-brand-muted sm:text-lg">
            {programme.overview.map((line, i) => (
              <p key={`${programme.slug}-o-${i}`}>{line}</p>
            ))}
          </div>
        </Container>
      </section>
      ...
      <section className="pb-14 sm:pb-16">
        <Container>
          ...
          <ul className="mt-6 max-w-4xl space-y-2 text-brand-muted sm:text-lg">
            {programme.keyFeatures.map((item, i) => (
              <li key={`${programme.slug}-f-${i}`}>• {item}</li>
            ))}
          </ul>
        </Container>
      </section>
        ...
        <Container>
          ...
          <div className="mt-6 space-y-4">
            {programme.faqs.map((faq, i) => (
              <article key={`${programme.slug}-faq-${i}`} className="rounded-2xl border border-brand-border/40 p-5">
